...
class TestTransaction(APITestCase):
    ...
    def test_status_authorization_and_len_response(self):
        ...
        # и колличество объектов в ответе (в данном случае ответ равен пагинации указанной в переданном параметре page_size)
    ...
    def test_transaction_creation(self):
        ...
        response = self.client.post(reverse('create_list_transactions'),
                                    {
                                        'user': self.user1.id,
                                        'amount': 111.11,
                                        'type': 'доход',
    ...
    def test_put_transaction_by_id(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.user_transaction.auth_token.key)
        response = self.client.put(reverse('get_put_delete_transaction_id', args=[self.transaction1.id]), {
            'amount': 999.99,
